...
export interface PriceDataAdapter {
  getTokenPrice(token: string): Promise<{ asset: string; priceUsd: string; timestamp: string }>;
}

export interface YieldAdapter {
  getYieldOpportunities(network: string): Promise<
    Array<{ asset: string; protocol: string; apy: number; tvlUsd: string; risk: 'LOW' | 'MEDIUM' | 'HIGH'; timestamp: string }>
  >;
}
 ...
 */
export function requireBlockchainRuntime(): never {
  throw new BANError(
    ErrorCode.INTERNAL,
    'Blockchain runtime is not configured. Set BAN_RPC_URL and BAN_CHAIN_ID to enable onchain adapters.',
    { retryable: false }
  );
}
