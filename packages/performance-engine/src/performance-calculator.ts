...
export class PerformanceCalculator {
  ...
  aggregatePositions(
    ...
    for (const pos of positions) {
      ...
      if (current > 0) managedCapital += current;
      ...
      if (pos.protocol === 'agent-wallet') continue;
  ...
  summarize(
    executions: Execution[],
    positions: Position[],
    opts?: { walletCapitalUsd?: string | null },
  ): {
    totalTrades: number;
    ...
    const walletCapitalUsd =
      opts?.walletCapitalUsd != null && Number(opts.walletCapitalUsd) > 0
