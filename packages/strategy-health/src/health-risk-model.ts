  ...
  stateFor(healthFactorCents: number): HealthRiskState {
    if (Number.isNaN(healthFactorCents) || healthFactorCents < 0) {
      throw new BANError(
        ...
        `Invalid health factor '${String(healthFactorCents)}' (fail-closed).`,
        { retryable: false },
      );
    }
    if (healthFactorCents >= HEALTH_THRESHOLDS.HEALTHY_MIN) return 'HEALTHY';
    if (healthFactorCents >= HEALTH_THRESHOLDS.WARNING_MIN) return 'WARNING';
    if (healthFactorCents >= HEALTH_THRESHOLDS.CRITICAL_MIN) return 'CRITICAL';
    return 'EMERGENCY';
  }
