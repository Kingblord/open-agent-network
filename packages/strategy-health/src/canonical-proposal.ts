...
export const HEALTH_VTOKENS: Record<string, string> = {
...
const VTOKEN_FUNCTIONS: Record<string, string> = {
  ADD_COLLATERAL: 'mint',
  REPAY: 'repayBorrow',
};
 ...
 */
export function canonicalizeHealthProposal(
  ...
  if (!candidate) return null; // no corrective candidate — honest no-op
  ...
  const finalHealthAction =
    typeof candidate.action === 'string' && VTOKEN_FUNCTIONS[candidate.action.toUpperCase()]
  ...
  const protocol =
    typeof candidate.protocol === 'string' && candidate.protocol
      ...
      : 'venus';
  if (protocol !== 'venus') return null; // unverified protocol → fail closed
  ...
  const candidateDenom =
    typeof candidate.denomination === 'string' ? candidate.denomination.trim().toUpperCase() : '';
  const modelAsset =
  ...
  const exactWei = toWeiIntegerString(
    typeof candidate.amountWei === 'string' ? candidate.amountWei : undefined,
  );
  ...
  if (!revalidated.success) {
    throw new BANError(
      ErrorCode.INTERNAL,
      `Health strategy produced an invalid canonical proposal: ${revalidated.error.message}`,
      { retryable: false },
    );
  }
