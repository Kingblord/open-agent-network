...
export function normalizeRiskLevel(value: unknown): unknown {
  if (typeof value !== 'string') return value;
  const key = value.trim().toUpperCase().replace(/\s+/g, '');
  const alias = RISK_ALIASES[key];
  if (alias) return alias;
  if (RISK_LEVELS.has(value.trim().toUpperCase())) return value.trim().toUpperCase();
...
export function normalizeProposalAction(
  ...
  const out: Record<string, unknown> = { ...proposal, action: canonical, params };
  ...
  for (const k of ['amount', 'estimatedValue']) {
    ...
    if (v == null) continue;
    if (typeof v === 'number') {
      if (!Number.isFinite(v) || v < 0) return proposal; // keep → fail closed
