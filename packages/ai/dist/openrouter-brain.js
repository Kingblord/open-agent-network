...
export function normalizeProposalAction(proposal) {
    const rawAction = typeof proposal.action === 'string' ? proposal.action.trim().toUpperCase() : '';
    // Non-executable directives are decisions to not trade, not onchain actions.
    if (rawAction === 'STOP' || rawAction === 'HOLD' || rawAction === 'WAIT')
        return null;
    const params = {
        ...(proposal.params ?? {}),
    };
    if (rawAction === 'BUY' || rawAction === 'SELL') {
        ...
        if (params.requestedAction == null)
