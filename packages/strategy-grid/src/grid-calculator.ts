...
export class GridCalculator {
  ...
  generateLevels(
  ...
  ): import('./types.js').GridLevel[] {
    if (gridCount < 2) throw new Error('gridCount must be >= 2');
    if (lowerPriceCents >= upperPriceCents) throw new Error('lowerPriceCents must be < upperPriceCents');
    if (capitalCents <= 0) throw new Error('capitalCents must be positive');
    ...
    const levels: import('./types.js').GridLevel[] = [];
    const medianIndex = Math.floor((gridCount - 1) / 2);

    for (let i = 0; i < gridCount; i++) {
      const priceCents = lowerPriceCents + spacing * i;
      let side: 'BUY' | 'SELL';
      if (i < medianIndex) side = 'BUY';
      else if (i > medianIndex) side = 'SELL';
      else side = 'BUY'; // median level — buy (conservative default)
   ...
   */
  detectCrossing(
    levels: import('./types.js').GridLevel[],
    previousPriceCents: number,
    currentPriceCents: number,
  ): import('./types.js').GridCrossing | null {
    if (previousPriceCents === currentPriceCents) return null;
   ...
   */
  computeOrderSizeCents(
    crossing: import('./types.js').GridCrossing,
  ...
  estimateProfitCents(
    ...
    _levels: import('./types.js').GridLevel[],
  ): number {
    if (orderSizeCents <= 0) return 0;
  ...
  shouldStop(priceCents: number, config: import('./types.js').GridConfig): boolean {
    if (config.stopOnLowerBoundBreak && priceCents < config.lowerPriceCents) return true;
    if (config.stopOnUpperBoundBreak && priceCents > config.upperPriceCents) return true;
    return false;
  }
}