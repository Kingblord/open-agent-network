...
export function eip7702Chain(env: Eip7702EnvLike = process.env): Chain {
  const rpc = env.BAN_RPC_URL?.trim();
  if (!rpc) {
    throw new BANError(
      ErrorCode.PROVIDER_UNAVAILABLE,
      'BAN_RPC_URL is not configured; cannot build an EIP-7702 mainnet chain',
      { retryable: false },
    );
  }
  ...
  if (chainId !== BAN_MAINNET_CHAIN_ID) {
    throw new BANError(
      ErrorCode.PROVIDER_UNAVAILABLE,
      `EIP-7702 executes on BNB MAINNET only (56); BAN_CHAIN_ID=${chainId} is not supported`,
      { retryable: false },
    );
  }
  return {
...
export function toAuthorizationList(auth: Eip7702Authorization): Eip7702AuthorizationTuple[] {
...
export function devTestingAccount(): {
  ...
  if (process.env.NODE_ENV === 'production') {
    throw new BANError(
      ErrorCode.INTERNAL,
      'DEV_PRIVATE_KEY is a dev/test-only key; refusing to use it in production (NODE_ENV=production)',
      { retryable: false },
    );
  }
  try {
    const account = privateKeyToAccount(key as Hex);
    const transport: Transport = http(eip7702Chain().rpcUrls.default.http[0]);
    ...
    throw new BANError(
      ...
      `DEV_PRIVATE_KEY is not a valid private key: ${err instanceof Error ? err.message : String(err)}`,
      { retryable: false },
    );
  }
}
...
export function agentWalletAccount(env: Eip7702EnvLike = process.env): {
  address: Address;
  walletClient: WalletClient;
} | null {
  ...
  try {
    const account = privateKeyToAccount(key as Hex);
    const transport: Transport = http(eip7702Chain(env).rpcUrls.default.http[0]);
    const walletClient = createWalletClient({ chain: eip7702Chain(env), transport, account });
    return { address: account.address, walletClient };
  } catch (err) {
    throw new BANError(
      ErrorCode.VALIDATION_FAILED,
      `Agent wallet private key is not valid: ${err instanceof Error ? err.message : String(err)}`,
      { retryable: false },
    );
  }
}
...
export async function submitAuthorization(input: {
  ...
  if (!input.submit) {
    throw new BANError(
      ErrorCode.INTERNAL,
      'No EIP-7702 submit backend provided; refusing to fabricate a transaction hash',
      { retryable: false },
    );
  }
