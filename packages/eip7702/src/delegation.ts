...
export function assertActivatable(perm: AgentPermission): void {
  // A permission is activatable only from the pre-authorization PENDING state.
  if (perm.status !== 'PENDING') {
    throw new BANError(
      ErrorCode.POLICY_DENIED,
      `Permission ${perm.id} is ${perm.status}, not PENDING; cannot activate`,
    );
  }
  if (!perm.validAfter || !perm.validUntil) {
    throw new BANError(
      ErrorCode.VALIDATION_FAILED,
      `Permission ${perm.id} has an invalid validity window`,
    );
  }
  const now = Date.now();
  ...
  if (now < after || now >= until) {
    throw new BANError(
      ErrorCode.POLICY_DENIED,
      `Permission ${perm.id} is outside its validity window [${perm.validAfter}, ${perm.validUntil})`,
    );
  }
 ...
 */
export function delegationForPermission(
  ...
  if (!perm.userAddress || !perm.validAfter || !perm.validUntil) {
    throw new BANError(
      ErrorCode.VALIDATION_FAILED,
      `Permission ${perm.id} is missing userAddress or validity window required to build a DelegationState`,
    );
  }
 ...
 */
export function assertAuthorityMatches(
  perm: AgentPermission,
  ...
  if (!perm.userAddress) {
    throw new BANError(
      ErrorCode.VALIDATION_FAILED,
      `Permission ${perm.id} is missing resolved owner (userAddress); cannot compare authority`,
    );
  }
  ...
  if (auth.address.toLowerCase() !== delegateAddress.toLowerCase()) {
    throw new BANError(
      ErrorCode.POLICY_DENIED,
      `Authorization delegates to ${auth.address}, not ${delegateAddress}`,
    );
  }
...
export function nextPermissionNonce(prev: string | undefined): string {
