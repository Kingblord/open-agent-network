...
export function toAuthorizationTuple(auth: Eip7702Authorization): Eip7702AuthorizationTuple {
  return [auth.chainId, auth.address, auth.nonce, auth.yParity, auth.r, auth.s];
}

/** Parse a raw tuple back to the typed object. */
export function fromAuthorizationTuple(tuple: Eip7702AuthorizationTuple): Eip7702Authorization {
 ...
 */
export function eip7702Digest(auth: Eip7702Authorization): string {
  const { chainId, address, nonce } = auth;
  const packed = encodePacked(
    ['uint256', 'address', 'uint256'],
    [BigInt(chainId), address as `0x${string}`, BigInt(nonce)],
  );
 ...
 */
export function delegationStateFromPermission(
  ...
  if (!perm.userAddress || !perm.validAfter || !perm.validUntil) {
    throw new BANError(
      ...
      `Permission ${perm.id} is missing userAddress or validity window required for a DelegationState`,
    );
  }
