 ...
 */
export function permissionConfigHash(perm) {
    if (!perm.agentId)
        throw new BANError(ErrorCode.VALIDATION_FAILED, 'permission is missing agentId');
    if (!perm.userAddress)
        throw new BANError(ErrorCode.VALIDATION_FAILED, 'permission is missing userAddress');
    ...
    const packed = encodePacked(['bytes', 'bytes'], [toHex(domainSep), toHex(joined)]);
 ...
 */
export function withConfigHash(perm) {
