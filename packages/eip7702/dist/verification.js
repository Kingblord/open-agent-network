...
export async function recoverAuthorizationSigner(auth, signature) {
    ...
    try {
        ...
        throw new BANError(ErrorCode.SCHEMA_INVALID, `Could not recover the authorization signer from the provided signature: ${err instanceof Error ? err.message : String(err)}`, { retryable: false });
    }
}
/**
 ...
 */
export async function verifyAuthorizationSigner(auth, signature, expectedUser) {
    if (!/^0x[a-fA-F0-9]{130}$/.test(signature)) {
        throw new BANError(ErrorCode.VALIDATION_FAILED, `Invalid 65-byte EIP-712-style signature: ${signature}`);
    }
    const digest = eip7702Digest(auth);
    try {
        const [rawSigner, prefixedSigner] = await Promise.all([
            recoverAddress({ hash: digest, signature: signature }),
            recoverAddress({ hash: hashMessage(digest), signature: signature }),
        ]);
        const expected = expectedUser.toLowerCase();
        return (rawSigner.toLowerCase() === expected ||
...
export async function verifyAuthorizationForPermission(input) {
    ...
    if (BigInt(input.auth.chainId) !== BigInt(expectedChain)) {
        throw new BANError(ErrorCode.POLICY_DENIED, `Authorization chain ${input.auth.chainId} does not match expected ${expectedChain}`);
    }
    if (!input.permission.userAddress) {
        throw new BANError(ErrorCode.POLICY_DENIED, `Permission ${input.permission.id} has no resolved owner (userAddress); cannot verify authorization`);
    }
    ...
    if (BigInt(input.auth.nonce) !== permissionNonce) {
        throw new BANError(ErrorCode.POLICY_DENIED, `Authorization nonce ${input.auth.nonce} does not match permission nonce ${input.permission.nonce}`);
    }
    return true;
}
