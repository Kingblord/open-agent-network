 ...
 */
const RISK_ADJUSTMENT_BPS = { LOW: 0, MEDIUM: 50, HIGH: 100 };
const COMPATIBLE = {
    LOW: ['LOW'],
    MEDIUM: ['LOW', 'MEDIUM'],
...
export class YieldRiskModel {
     ...
     */
    apply(normalized, agentRisk) {
        const out = [];
        for (const n of normalized) {
            const adjustment = RISK_ADJUSTMENT_BPS[n.risk];
            if (adjustment === undefined) {
                throw new BANError(ErrorCode.POLICY_DENIED, `Unknown risk '${String(n.risk)}' cannot be ranked (fail-closed).`, { retryable: false });
            }
            if (!this.compatible(agentRisk, n.risk))
                continue; // drop incompatible (not an error)
            out.push({
                ...n,
