 ...
 */
function findKeyValue(node: Record<string, unknown>, key: string): unknown {
  const direct = Object.keys(node).find((k) => k.toLowerCase() === key.toLowerCase());
  return direct === undefined ? undefined : node[direct];
}

function deepAddressLookup(node: unknown, key: string): string | null {
  ...
  if (entry !== undefined) {
    if (isAddressLike(entry)) return entry;
    if (isRecord(entry) && isAddressLike(entry.address)) return entry.address;
    if (Array.isArray(entry)) {
      for (const item of entry) {
        if (isAddressLike(item)) return item;
        if (isRecord(item) && isAddressLike(item.address)) return item.address;
      }
    }
  }
...
function broadcastLookup(payload: unknown, role: string): string | null {
  if (!isRecord(payload) || !Array.isArray(payload.transactions)) return null;
  const roleLower = role.toLowerCase();
  for (const tx of payload.transactions) {
    if (!isRecord(tx)) continue;
    const name = typeof tx.contractName === 'string' ? tx.contractName.toLowerCase() : '';
    if (name !== roleLower) continue;
...
export function findRoleAddress(payload: unknown, role: string): string | null {
...
export function resolveRolesFromPayload(
  protocolId: string,
  references: ProtocolRoleReference[],
  payload: unknown,
  sourceUrl: string
): ResolvedRoleAddress[] {
...
function candidateUrls(protocolId: 'aave' | 'lista'): string[] {
  const env =
    protocolId === 'aave'
...
const PANCAKE_V3_BNB_ROLES: ProtocolRoleReference[] = [
  {
...
export async function fetchAndResolveProtocol(
  protocolId: 'aave' | 'lista' | 'pancakeswap'
): Promise<FetchOutcome> {
  let refs: ProtocolRoleReference[];
  let urls: string[];
...
export async function fetchAllProtocolOutcomes(): Promise<FetchOutcome[]> {
...
async function writeFetchedCandidates(outcomes: FetchOutcome[]): Promise<void> {
  ...
  for (const o of outcomes) {
    if (!o.ok) {
      ...
      for (const r of o.roles) {
        log(
          `  EMPTY  ${r.role.padEnd(22)} ${r.usedByAdapterAs.padEnd(18)} no published address resolved — fail-closed (never guessed)`
        );
      }
      continue;
    }
