...
import { deriveProtocolIntegrationStatus, INTEGRATION_LADDER } from './integration-status.js';
/**
 ...
 */
export function buildBnbRegistrySnapshot(chainId = BNB_CHAIN_ID) {
    ...
    const contractsByProtocol = new Map();
    ...
    const protocolSnapshot = protocols
        .list()
        .map((p) => {
        const records = contractsByProtocol.get(p.id) ?? [];
        const ladder = deriveProtocolIntegrationStatus(records.map((r) => ({
            verified: r.verified,
            enabled: r.enabled,
        ...
        const pa = pOrder[(a.priority ?? 'P3')] ?? 3;
        const pb = pOrder[(b.priority ?? 'P3')] ?? 3;
        if (pa !== pb)
            return pa - pb;
