...
const LP_FUNCTIONS: Record<string, string> = {
...
const LP_ACTION_TO_CANONICAL: Record<string, ActionProposal['action']> = {
  REMOVE: 'BURN',
  CREATE: 'MINT',
 ...
 */
export function canonicalizeLpProposal(
  ...
  const obsData = (observation as { data?: Record<string, unknown> }).data ?? {};
  const poolAddress = typeof obsData.poolAddress === 'string' ? obsData.poolAddress : '';
  if (!isHexAddress(poolAddress)) return null; // no verified pool — refuse

  const candidates = readObservationCandidates<Record<string, unknown>>(observation);
  const candidate = pickCandidate(candidates, {
    side: rawAction,
    tradeActions: ['REMOVE', 'CREATE', 'REPOSITION'],
  });
  if (!candidate) return null; // no LP candidate — honest no-op
  ...
  const position = obsData.position as { liquidity?: string } | null | undefined;
  const amount =
    lpAction === 'REMOVE'
      ? toWeiIntegerString(position?.liquidity ?? proposal.amount)
      : toWeiIntegerString(proposal.amount);
  if (amount == null) return null; // refuse to spend/mint blindly

  const enriched: ActionProposal = {
    ...proposal,
    action: canonical,
    protocol: 'pancakeswap',
    contract: poolAddress as `0x${string}`,
  ...
  if (!revalidated.success) {
    throw new BANError(
      ErrorCode.INTERNAL,
      `LP strategy produced an invalid canonical proposal: ${revalidated.error.message}`,
      { retryable: false },
    );
  }
