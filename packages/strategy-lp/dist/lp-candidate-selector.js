...
export class LpCandidateSelector {
    ...
    select(pool, position) {
        ...
        candidates.sort((a, b) => {
            if (a.action === 'REMOVE' && b.action !== 'REMOVE')
                return -1;
            if (b.action === 'REMOVE' && a.action !== 'REMOVE')
                return 1;
    ...
    estimateProjectedFees(pool, position, range, width) {
