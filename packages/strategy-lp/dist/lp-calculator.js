...
export class LpRangeCalculator {
    ...
    sqrtPriceX96ToTick(sqrtPriceX96) {
        ...
        if (s <= 0n)
            throw new BANError(ErrorCode.VALIDATION_FAILED, 'sqrtPriceX96 must be positive');
     ...
     */
    generateCandidateRanges(currentTick, tickSpacing, maxCandidates = 5) {
        const candidates = [];
     ...
     */
    estimateNetProfitCents(projectedFeesCents, gasCostCents, slippageCents, riskAdjustmentCents) {
     ...
     */
    shouldRebalance(currentTick, lowerTick, upperTick, netProfitCents) {
        if (this.isInRange(currentTick, lowerTick, upperTick))
            return false;
    ...
    token0FractionBps(currentTick, lowerTick, upperTick) {
