...
export function WalletConnectCard() {
  ...
  return (
    <div className="bg-card rounded-xl p-5 border border-border">
      <h3 className="text-[10px] font-black text-foreground tracking-widest uppercase mb-4">
        Connected Wallet
      </h3>

      {hydrating ? (
        <p className="text-xs text-muted-foreground">Checking saved wallet…</p>
      ) : displayAddress ? (
        <div className="space-y-3">
          <div className="flex items-center gap-3 p-3 bg-muted/60 border border-border rounded-lg">
            ...
            <div className="flex-1 min-w-0">
              <div className="font-mono text-sm text-foreground truncate">{shortAddress(displayAddress)}</div>
              <div className="text-[10px] text-muted-foreground font-mono truncate">{displayAddress}</div>
            </div>
            <div className="text-[9px] font-black uppercase tracking-wider text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 border border-emerald-500/30 rounded px-2 py-1">
              Linked
            </div>
          </div>
          ...
          <button
            ...
            className="text-[10px] font-black uppercase tracking-wider text-red-500 dark:text-red-400 hover:text-red-400 dark:hover:text-red-300 border border-red-500/30 dark:border-red-400/30 rounded px-3 py-2 hover:bg-red-500/10 transition"
          >
            Disconnect wallet
          </button>
        </div>
